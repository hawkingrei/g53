...
func (s *DNSServer) AddService(id string, service Service) {
	if (service.Record_type == "CNAME" || service.Record_type == "A"){
		...
		logger.Warningf("Service '%s' ignored: No Record_type provided:", id, id)
	}
}
...
// one exception to accomodate the desired behavior we wish from dtdns,
